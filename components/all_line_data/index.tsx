...
// console.log(priceShowType,'priceShowType');
  ...
  const initChart = React.useCallback(
    (data) => {
      ...
      chart.current.scale('floor_price', {
        nice: true,
      });
      ...
      chart.current
        ...
        .tooltip(
          'time*floor_price*name*staticT*priceStaticT',
  ...
  const initChartUsd = React.useCallback(
    (data) => {
      ...
      chart.current.scale('floor_price_usd', {
        nice: true,
      });
      chart.current.tooltip({
        // showMarkers: false,
      ...
      // chart.current.interaction('legend-highlight');
      // 设置横纵轴
      chart.current.axis('floor_price_usd', {
        grid: {
          line: {
            type: 'line',
            style: (x, y) => {
      ...
      chart.current
        ...
        .tooltip(
          'time*floor_price_usd*name*staticT*priceStaticT',
          (time, value, name, staticT, priceStaticT) => {
            return {
              value,
              time,
  ...
  const updata = React.useCallback(
    (st, pt) => {
      if (chart.current && dataSource[0]) {
        chart.current.changeData(transfromData(dataSource[st][pt], st));
      }
    },
    [dataSource],
  );
  ...
  }, [requestData,iconImgLight,priceShowType]);
