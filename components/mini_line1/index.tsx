...
export default function MiniLine1({ id, labelText, dataHandlder, iconImgLight,legend1, legend2, limit,textColor,HyperlinkJump }: Props) {
  ...
  const requestData = React.useCallback(async () => {
    ...
    setLoading(false);
    if (result&&iconImgLight===false) {
      initChart(result);
    }else{
      initChart1(result);
    }
    return result;
  }, [dataHandlder,iconImgLight]);
  ...
  }, [loading, error, onRetry,iconImgLight]);
  ...
  return (
    <div className={cn('',iconImgLight===true?style.container1:style.container)}>
      <div className={cn('w-full flex justify-between item-center', style.header)}>
        ...
        <div className={cn("flex items-center")}>{getLenged}</div>
      </div>
      {render}
    </div>
  );
