...
// last_price.tez	float	地块最后一次交易金额对应的tez
...
export default function ParcelDeatil({
  ...
  const getLabel = (type, op) => {
    ...
    const label = op.timeRangeSale?.tez
      ? `${op.timeRangeSale.tez} TEZ(${op.timeRangeSale.usd} USD)`
      : '';
    if (type === 'ALL') {
            ...
            {list.map((ite, idx) => {
              return (
                <li key={idx} className={cn('', styles.sales)}>
                  ...
                  <span className="text-white">{`${ite.tez} TEZ(${ite.usd} USD)`}</span>
                </li>
              );
            })}
          </>
