...
export const req_wearableSandBox_list = async (page: number, count: number,
  ) => {
  const search = qs.stringify({ page, count }, { addQueryPrefix: true });
  const url = `/api/get_wearableSandBox_list${search}`;
  const result = await fetch(url, {
    method: 'get',
    mode: 'cors',
  });
...
export const req_detailWearableSandBox_list = async (avatar_id,
  ) => {
  const search = qs.stringify({  avatar_id, }, { addQueryPrefix: true });
  const url = `/api/get_wearableSandBoxDetail_list${search}`;
  const result = await fetch(url, {
    method: 'get',
    mode: 'cors',
  });
